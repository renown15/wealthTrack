/** API service facade — extends the hubs layer with groups, analytics, and share sale */
import type {
  PortfolioBreakdown,
  PortfolioHistory,
...
import { ApiServiceHubs } from '@services/ApiServiceHubs';

class ApiService extends ApiServiceHubs {
  ...
  async recordActualCost(accountId: number, data: ActualCostCreateRequest): Promise<ActualCostItem> {
    return outgoingCostService.recordActualCost(accountId, data);
  }

  async listActualCosts(accountId: number): Promise<ActualCostItem[]> {
    return outgoingCostService.listActualCosts(accountId);
  }

  async deleteActualCost(groupId: number): Promise<void> {
    return outgoingCostService.deleteActualCost(groupId);
  }

  async getOutgoingProjections(): Promise<OutgoingProjection[]> {
    return outgoingCostService.getProjections();
  }
}
