"""Service for managing RSU balance snapshots."""
...
class RSUBalanceService:
    """Service for saving and managing RSU balance snapshots."""
    ...
    async def save_daily_balance(
        ...
        """
        Save RSU balance (with 47% tax applied) as account attribute.
        ...
        Args:
            ...
            user_id: User ID
            balance_value: Balance in pounds after 47% tax (float)
        ...
        try:
            ...
            # Get "RSU Balance" attribute type ID using the repository method
            repo = AccountAttributeRepository(session)
            attr_type_id = await repo.get_attribute_type_id("rsu_balance")
            ...
            if existing_event:
                logger.debug(
                    f"[RSUBalanceService] Balance event already saved today for "
            ...
            logger.info(
                f"[RSUBalanceService] Saved RSU balance for "
            ...
            logger.error(
                f"[RSUBalanceService] Error saving balance for account {account_id}: {e}",
                exc_info=True,
            )
            return False
