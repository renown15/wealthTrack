...
@router.post("/{family_id}/members/{member_id}/accounts/{account_id}/events")
async def create_member_event(
    ...
    """Create a balance event for a family member's account."""
    if not await FamilyService(session).verify_membership(family_id, member_id, current_user.id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not a family member")
    if not await AccountRepository(session).get_by_id(account_id, member_id):
    ...
    event = await event_repo.create_event(
        account_id=account_id, user_id=member_id, type_id=type_id, value=event_data.value,
    )
